...
bool antiCheck(LIST *nodes, int xp, int yp, int y, int x) {
    if (yp >= 0 && yp < y && xp >= 0 && xp < x && !contains(nodes, xp, yp)) {
...
int main() {
    ...
    for (int i = 1; i < RANGE; i++) {
        ...
        while (start != NULL && start->next != NULL) {
            ...
            while (end != NULL) {
                // printf("e%p\n", end);
                if (antiCheck(antenas[0], 2*(end->x) - start->x, 2*(end->y) - start->y, y, strlen(line)-1)) {
                    count++;
                }
                if (antiCheck(antenas[0], 2*(start->x) - end->x, 2*(start->y) - end->y, y, strlen(line)-1)) {
                    count++;
                }
                end = end->next;
            }
